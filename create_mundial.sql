...
CREATE FUNCTION   IsReplacementValid(@IDSummonedPlayerOut INT, @IDSummonedPlayerIn INT, @IDMatch INT)
RETURNS BIT
AS
...
BEGIN
   ...
   FROM INSERTED I JOIN MATCH M ON I.IDMATCH = M.IDMATCH
                    JOIN COUNTRIES C ON M.IDAWAYTEAM = C.IDCOUNTRY
                    JOIN PLAYER P ON I.IDPLAYER = P.IDPERSON
                    JOIN SUMMONEDS S ON I.IDSUMMONED = S.IDSUMMONED
   WHERE P.IDCOUNTRY <> M.IDAWAYTEAM AND P.IDCOUNTRY <> M.IDHOMETEAM;

    DECLARE @NumRejectedPlayers INT; --This variable is used to store the count of rows affected preivously
    SET @NumRejectedPlayers = @@ROWCOUNT; --it's the number of rows deleted by the DELETE statement in the trigger.

    IF @NumRejectedPlayers > 0 --If there are rows affected, then print the message
        PRINT 'Some players were not summoned since they did not belong to any of the countries participating in the match';
END;
GO

--2 A referee cannot referee a match where her/his own country is participating. (INSERT/UPDATE)
-- INSERT Trigger for Referees
CREATE OR ALTER TRIGGER after_insert_referee_country
ON REFEREE
AFTER INSERT
AS
BEGIN
    DECLARE @TotalRecords INT; --This variable is used to store the count of rows affected preivously
    DECLARE @NumDeletedRecords INT; --This variable is used to store the count of rows affected preivously

    DELETE REFEREE
    FROM inserted i
    JOIN MATCH M ON i.IDPERSON = M.IDREFEREE
    JOIN COUNTRIES C ON i.IDCOUNTRY = C.IDCOUNTRY
    WHERE C.IDCOUNTRY = M.IDAWAYTEAM OR C.IDCOUNTRY = M.IDHOMETEAM

    SET @NumDeletedRecords = @@ROWCOUNT; --it's the number of rows deleted by the DELETE statement in the trigger.
    SET @TotalRecords = (SELECT COUNT(*) FROM inserted); --it's the number of rows inserted by the INSERT statement in the trigger.

    IF @NumDeletedRecords < @TotalRecords --If there are rows affected, then print the message
        PRINT cast((@TotalRecords - @NumDeletedRecords) as nvarchar(10)) + ' row(s) inserted';
    
    IF @NumDeletedRecords > 0 --If there are rows affected, then print the message
        PRINT 'There are (' + cast((@NumDeletedRecords) AS nvarchar(5)) + ' rows) where the referee cannot referee a match involving their own country, thus they were not inserted';
END;
GO

-- UPDATE Trigger for Referees
CREATE OR ALTER TRIGGER after_update_referee_country
ON REFEREE
AFTER UPDATE
AS
BEGIN
    IF TRIGGER_NESTLEVEL() > 1 --This is to avoid infinite recursion
        RETURN

    DECLARE @TotalRecords INT; --This variable is used to store the count of rows affected preivously
    DECLARE @NumIncorrectRecords INT; --This variable is used to store the count of rows affected preivously
    ...
    SET
        ...
        GENDER = d.GENDER
    FROM deleted d
    JOIN inserted i ON d.IDPERSON = i.IDPERSON
    JOIN MATCH M ON i.IDPERSON = M.IDREFEREE
    JOIN COUNTRIES C ON i.IDCOUNTRY = C.IDCOUNTRY
    WHERE C.IDCOUNTRY = M.IDAWAYTEAM OR C.IDCOUNTRY = M.IDHOMETEAM

    SET @NumIncorrectRecords = @@ROWCOUNT; --it's the number of rows deleted by the DELETE statement in the trigger.
    SET @TotalRecords = (SELECT COUNT(*) FROM inserted); --it's the number of rows inserted by the INSERT statement in the trigger.

    IF @NumIncorrectRecords < @TotalRecords --If there are rows affected, then print the message
        PRINT cast((@TotalRecords - @NumIncorrectRecords) as nvarchar(10)) + ' row(s) updated'; --If there are rows affected, then print the message
    
    IF @NumIncorrectRecords > 0 --If there are rows affected, then print the message
        PRINT 'There are (' + cast((@NumIncorrectRecords) AS nvarchar(5)) + ' rows) where the referee cannot referee a match involving their own country, thus they were not updated';
END;
GO
...
BEGIN
    DECLARE @TotalRecords INT; --This variable is used to store the count of rows affected preivously
    DECLARE @NumYellowCards INT; --This variable is used to store the count of rows affected preivously
   ...
   FROM inserted I JOIN EVENTS E ON I.IDSUMMONEDMAINPLAYER = E.IDSUMMONEDMAINPLAYER
                                  AND I.MATCHPART = E.MATCHPART
                                  AND I.CARDTYPE = 'Yellow'
   -- Check if the player was in the starting 11
   WHERE NOT EXISTS (
        SELECT 1 -- Select 1 to avoid returning all the columns
        FROM STARTING_11 S 
        WHERE S.IDSUMMONEDMAINPLAYER = I.IDSUMMONEDMAINPLAYER
          AND S.MATCHPART = I.MATCHPART
   );

    SET @NumYellowCards = @@ROWCOUNT; --it's the number of rows deleted by the DELETE statement in the trigger.
    SET @TotalRecords = (SELECT COUNT(*) FROM inserted); --it's the number of rows inserted by the INSERT statement in the trigger.

    IF @NumYellowCards > 0 --If there are rows affected, then print the message
        PRINT 'Some players were given a red card since they already had a yellow card in the same match';
END;
GO
--4
CREATE OR ALTER TRIGGER goal_event
ON EVENTS
AFTER INSERT
AS
BEGIN
    ...
    FROM inserted I JOIN SUMMONEDS S ON I.IDSUMMONEDMAINPLAYER = S.IDSUMMONED
    WHERE dbo.IsPlayeronField(I.IDSUMMONEDMAINPLAYER, I.MATCHPART) = 1

    SET @NumIncorrectRecords = @@ROWCOUNT; --it's the number of rows deleted by the DELETE statement in the trigger.
    SET @TotalRecords = (SELECT COUNT(*) FROM inserted); --it's the number of rows inserted by the INSERT statement in the trigger.

    IF @NumIncorrectRecords < @TotalRecords --If there are rows affected, then print the message
        PRINT cast((@TotalRecords - @NumIncorrectRecords) as nvarchar(10)) + ' row(s) inserted'; --If there are rows affected, then print the message
    
    IF @NumIncorrectRecords > 0 --If there are rows affected, then print the message
        PRINT 'There are (' + cast((@NumIncorrectRecords) AS nvarchar(5)) + ' rows) where the player that scored the goal was not on the field, thus they were not inserted';
END;     
GO
